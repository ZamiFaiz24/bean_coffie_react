  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((card, idx) => {
        ...
        const colorClass = colorMap[card.type] || 'text-gray-700';
        const bgColorClass = bgColorMap[card.type] || 'bg-gray-50';
        const TrendIcon = card.trend > 0 ? TrendingUp : TrendingDown;
        const trendColor = card.trend > 0 ? 'text-emerald-600' : 'text-red-600';

        return (
          <Card key={idx} className="border-gray-200 bg-white hover:shadow-card transition-shadow duration-300">
